...
step2: 以下の操作を20回繰り返す
step2-1: グラフGの頂点に関する最短経路問題を50回解く
step2-2: 過去すべての経路情報を用いて、グラフGの辺に関する線形回帰モデルを作る
step2-3: モデルの重みを平滑化し、グラフGの新たな辺の重みとする

predicted score:
95.0% confidence interval: 931975974 [928813056, 935138892]

atcual score:
93145002649

https://atcoder.jp/contests/ahc003/submissions/22957630
"""
...
def solve(N = 1000, M = 30, INF=100000000):
    BATCH = 50
    regressor = Ridge(tol=1e1)
    X = np.empty((N+1, M*M*2), dtype=np.float32)
    Y = np.empty(N+1, dtype=np.float32)
    G = init_graph(M)
    X[0, :] = 1
    Y[0] = regressor.coef_.sum() if regressor and hasattr(regressor, "coef_") else 5000.0*M*M*2
    ...
    for k in range(N // BATCH):
        ...
        for i in range(BATCH):
            ...
            Y[k*BATCH+i+1] = cost2y(actual_cost)
